...
describe("E2E workbench image contract", () => {
  it("uses only the reviewed linux/amd64 Node base", () => {
    assert.equal(
      dockerfile.match(
        new RegExp(
          `FROM --platform=linux/amd64 docker\\.io/library/node@sha256:${baseDigest}`,
          "g"
        )
      )?.length,
      2
  ...
  it("binds provenance and deterministic build inputs", () => {
    assert.match(
      dockerfile,
      /ARG SOURCE_REVISION[\s\S]*test "\$\{#SOURCE_REVISION\}" -eq 40/
    );
    assert.match(
      dockerfile,
      /ARG SOURCE_DATE_EPOCH[\s\S]*case "\$\{SOURCE_DATE_EPOCH\}" in \*\[!0-9\]\*\)/
    );
    assert.match(
      dockerfile,
  ...
  it("runs only the isolated staging workbench as a non-root user", () => {
    assert.match(dockerfile, /USER 65532:65532/);
    assert.match(
      dockerfile,
      /ENTRYPOINT \["node", "\/app\/e2e-workbench\.cjs"\]/
    );
