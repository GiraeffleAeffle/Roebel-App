...
export function renderComposeYml(m: NetizenManifest): string {
  ...
  if (hasNostr) {
    ...
    image: ghcr.io/hoytech/strfry:latest
    restart: unless-stopped
    entrypoint: ["/bin/sh", "/etc/federation/sync-peers.sh"]
    volumes:
      - "./strfry.conf:/etc/strfry.conf"
      - "./federation:/etc/federation:ro"
      - "strfry_db:/app/strfry-db"
    depends_on: [strfry]`,
      );
    }
...
export function renderFederationSync(m: NetizenManifest, intervalSeconds = 300): string {
  ...
  const lines = peers.map((p) => {
    ...
    return [
      `  echo "[$(date -u +%FT%TZ)] sync ${p.id} (${p.direction}) ${p.relay}"`,
      `  strfry sync "${p.relay}" --dir ${p.direction} --filter '${filter}' \\`,
...
# Mirrors the PUBLIC record of declared peers. Every peer, kind and direction
# below comes from the manifest, so this file is never the place to edit trust.
set -u
...
export function renderFederationPeers(m: NetizenManifest): string {
  const peers = m.peers ?? [];
  const rows = peers
    .map((p) => `| ${p.id} | ${p.name} | ${p.relay} | ${p.kinds.join(", ")} | ${p.direction} | ${p.why} |`)
    .join("\n");
...
| id | name | relay | kinds | direction | why |
|---|---|---|---|---|---|
${rows || "| _none_ | | | | | |"}
`;
}
...
export function renderBundle(m: NetizenManifest): Bundle {
  ...
  if (m.services.chat?.nostr) {
    ...
    if (m.peers?.length) {
      files["federation/sync-peers.sh"] = renderFederationSync(m);
      files["federation/PEERS.md"] = renderFederationPeers(m);
    }
  }
  if (m.ai?.selfHosted) {
