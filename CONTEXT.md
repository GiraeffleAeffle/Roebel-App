...
| **Civic promotion** | A human-confirmed action that creates a civic topic from a source post, attaches the post to an existing topic, starts a structured discussion, or advances reviewed material to a signed suggestion. | Automatic classification by Mecky, proof of citizenship, or an administrative admission. |
...
| **Public civic projection** | The product-owned, read-only view that links ordinary Röbel posts to verified public topic, discussion, Mecky, suggestion, case, administration, and participation records. It may hide unavailable stages but cannot create or advance them. | The staging workbench, a write API, a Civic Case source of truth, or municipal authority. |
...
| **Participant topic suggestion** | A staging participant's signed, topic-bound suggestion linking the exact discussion and reviewed Mecky receipt while awaiting a separate citizen-credential adoption. | A citizen-signed proposal candidate, proof of residency, a Civic Case, or municipal authority. |
| **Municipal civic eligibility receipt** | A public-safe, municipality-policy-bound issuer statement that one Nostr key may adopt one exact participant suggestion for Case Steward review during a bounded time. | A public resident directory, app login, Case admission, voting right, municipal decision, or treasury authority. |
| **Citizen adoption** | A new, versioned signed transition in which a verified citizen credential references and adopts an immutable participant suggestion, includes a public-safe eligibility receipt reference, and creates the citizen-signed improvement proposal candidate. | Relabelling the participant event, the existing direct `citizen_signed_topic_suggestion_v1` shape, Case admission, a vote, or a municipal decision. |
| **Synthetic adoption tracer** | A separately typed, explicitly non-authoritative staging candidate and isolated Case path used to test journey UX before civic-eligibility verification exists. | Citizen adoption, a citizen-signed proposal candidate, the real Case Steward validator, production authority, voting, or treasury rights. |
...
| **Municipal publication** | A jurisdiction-scoped public record issued by an authorized municipal producer after a human municipal transition. It carries issuer identity, jurisdiction, source/session/work-package references, version, checksum, lifecycle and explicit `official_record` authority. | Raw meeting context, an AI summary, a Röbel post, automatic Civic Case admission, or proof of execution beyond what the record states. |
| **Municipal publication producer** | A municipality-operated Kair/openDesk/RIS adapter—or another explicitly approved system—that may issue an attributable Municipal publication. | Mecky, the public app, Stadtstack validation/projection, or a generic protocol consumer. |
| **City-neutral civic exchange** | The versioned boundary through which Municipal publications and other civic records can be projected as OParl-compatible records, queried through MCP, and replicated through signed Nostr/Netizen events without embedding one city's identifiers. | A single frontend, a replacement for municipal authority, or a Röbel/Strausberg-specific database. |
...
| **Administrative work package** | The idempotently exported, human-owned task created in a municipality-operated Kair/openDesk workspace only after proposal admission. | A raw feed post, Mecky answer, automatic municipal decision, or official publication. |
...
| **Staging participant session** | A short-lived, wallet-proved capability allowing one admitted tester to create constrained text posts and comments in the ordinary staging feed. | Citizen verification, residency, organisation control, Nostr publication, voting, treasury, Civic Case admission, or municipal authority. |

An ordinary feed post has no mandatory lifecycle. A civic journey begins only after an explicit human promotion:

`Feed post ──human promotion──> Civic topic → Discussion → Mecky answer → Participant suggestion (when needed) → Citizen adoption/signature → Human admission → Civic case → Municipal workspace work (Kair/openDesk or equivalent) → Official publication + receipt → Citizen Brief → Advisory Mitmachen signal → (separate authority gate) Governance ballot → (separate treasury gate) execution → public outcome`.
