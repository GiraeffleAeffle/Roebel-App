...
After isolated staff admission, Röbel reads the separate
`public_synthetic_case_binding_receipt_v1` through the existing credential-free
Case-binding BFF. Its checksum binds the synthetic Case namespace, exact test
proof and tracer, participant suggestion, discussion, Mecky answer, original
acceptance checksum and test policy. Closed staging/test-only fields prohibit
eligibility or civic-effect claims. The browser correlates this receipt with
the unchanged public preview and signed suggestion before displaying
“Isolierter Testfall aufgenommen”. A public receipt may recover its public
preview without login or a new signature. Missing, unavailable or mismatched
receipts do not advance the test state.

This receipt never enters the municipal proposal binder, topic journey,
administration reader or participation path, including when the proposal
projection is absent. The original preview still reports no CivicCase and no
workflow submission. Test department work remains a separate, unimplemented
continuation. The offline interoperability vector was generated by the neutral
[Stadtstack writer](https://github.com/GiraeffleAeffle/stadtstack/tree/02d10db3a68c37c66f78c7c558554ca0201525be)
from its public synthetic fixture; it contains no live operational records.
