...
Mitmachen reads that brief through the same exact case-bound, GET-only public projection as the Civic Journey. It does not load a detached “public profile.” The reviewed finance package may be displayed as budget context with its package binding and public citations, but it is not a treasury balance, funding commitment, reservation, or execution instruction.
...
- the Mitmachen link carries the exact canonical CivicCase identity from the journey, and its finance context remains labelled as reviewed administration feedback;
