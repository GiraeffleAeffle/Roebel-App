...
- **Remaining onboarding work:** let each intended tester control a credential,
  ...
  acceptance proof. A bridge from the existing app login into the staging issuer
  is now prepared and locally tested for people without a browser wallet; both
  Web and issuer images still need activation before that path is available.
...
The same source candidate prepares the existing-app login path for the Workspace.
The issuer opens a dedicated Röbel confirmation page; a user gesture signs only
its fixed, short-lived SIWE request through `CitizenSession`. Both windows check
their peer and origin, and the issuer retains its current nonce, signature and
allowlist checks. Staff grants are separate. Two fixture credentials exercise the
complete exchange locally; real app sign-in, callback, recovery and separately
controlled people remain live acceptance work. See
[ADR 0014](../adr/0014-provider-neutral-member-identity-and-staged-wallet-migration.md#staging-workspace-login-through-the-existing-app-account--2026-09-15).
