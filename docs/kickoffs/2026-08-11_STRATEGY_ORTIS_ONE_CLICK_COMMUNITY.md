...
1. ~~**Backend per tenant.**~~ **DECIDED 2026-08-11 (Max), then REVISED the same day → a tenant database on a Netizen node, not an external vendor project. See [K5](2026-08-11_K5_SOVEREIGN_DATA_PLANE.md), which supersedes the paragraph below.** Original reasoning, kept for the record: one Supabase project per community. Clean data isolation, per-community DSGVO controllership, straightforward deletion. Costs: the pipeline must provision via the Supabase Management API, and — combined with the self-serve signup decision — **every signup provisions a paid resource**, so a launch policy gate is mandatory (see [K4 §4.1](2026-08-11_K4_ORTIS_DASHBOARD.md)). A record-only free tier ([K2](2026-08-11_K2_NOSTR_READ_FALLBACK.md)) remains the candidate safe default for unpaid signups.
