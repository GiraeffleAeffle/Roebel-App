...
| 7. Identity coexistence              | A Thirdweb tester and a passkey/Safe tester use the same `CitizenSession` contract; one dual-proof link preserves an existing app account and npub | provider-neutral Safe adapter plus short-lived dual-control proof envelope implemented; passkey creation/recovery, stable-member persistence, nonce consumption, signature verification, Nostr-key recovery, and deployed opt-in E2E remain |
