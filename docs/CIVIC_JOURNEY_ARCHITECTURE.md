...
| [ADR 0022](adr/0022-author-confirmed-topic-candidate-tracer.md)                       | accepted; source implementation complete, staging activation pending | Promote one existing signed source note through two closed author-only operations into a topic root and participant suggestion; citizen adoption remains a separate identity transition. |
| [ADR 0023](adr/0023-city-neutral-citizen-eligibility-and-suggestion-adoption.md)      | accepted; kernel, browser signer and read-side handoff implemented; writer wiring pending | Verify municipality-specific eligibility behind a city-neutral public receipt, then let an eligible citizen adopt an immutable participant suggestion without gaining Case, voting, or treasury authority. |
...
| 2. Explicit civic promotion          | The user attaches that post to a topic or creates a discussion; the source remains attributable                                                    | browser-signed author-only promotion, grouped feed activity, and a canonical topic hub are implemented. The current product candidate also resolves the source post back to exactly one journey and shows its next stage on both the normal post detail and labelled Nostr fallback; conflicting bindings fail closed. ADR 0022's two closed writer operations end in a `citizen_adoption_required` participant suggestion, with durable exact-envelope replay, one-source/one-root claims, exact conversation provenance, and fail-closed source-row/ACL/RLS attestation. Staging activation and a connected-account browser acceptance run remain. |
...
| 4. Citizen adoption, proposal, and admission | A verified citizen credential adopts/signs the suggestion; a separately authenticated Case Steward admits one idempotent civic case and the public journey reads its binding receipt | The provider-neutral session now signs the existing ADR-0023 adoption kernel, and Röbel's v2 public receipt contract fail-closed binds participant, adoption, eligibility, ledger acceptance and Case admission before administration can load. The browser visibly keeps participant signature, citizen adoption and Case Steward admission separate. The issuer adapter, authenticated issuance challenge, adoption gateway/ledger, role-isolated Case writer/control service, staff console, upstream v2 projection, GitOps resources and deployed browser E2E remain. Existing direct `citizen_signed_topic_suggestion_v1` is not an adoption event. A separately typed synthetic adoption/Case tracer may test the UI but cannot satisfy this slice. |
...
| 6. Brief, participation, and finance | Citizen Brief, advisory Mitmachen, and treasury review are visible together without implying authority                                             | exact case-bound Mitmachen reader and reviewed finance context implemented. The current product candidate keeps a withdrawn Citizen Brief visibly attributable while gating Mitmachen until a new checksum-bound brief exists. Participation input/result and deployed browser proof remain. |
