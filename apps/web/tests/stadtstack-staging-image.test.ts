...
const dockerfile = readFileSync(
  new URL("../../../Dockerfile.staging-web", import.meta.url),
  "utf8"
);
const dockerignore = readFileSync(
  new URL("../../../Dockerfile.staging-web.dockerignore", import.meta.url),
  "utf8"
);
...
test("emits the standalone server only for the explicit Talos staging image", () => {
  ...
  assert.doesNotMatch(dockerfile, /NEXT_PUBLIC_STADTSTACK_PUBLIC_BASE_URL/);
});
...
test("sends only the web app and its exact workspace graph to the staging builder", () => {
  ...
  for (const path of [
    ...
    assert.match(
      dockerignore,
      new RegExp(`^!${path.replaceAll("/", "\\/")}\\/\\*\\*$`, "m")
    );
  }
  for (const forbidden of [
    "apps/expo",
    "apps/mini-apps",
    "circles-roebel-mini-app",
    "contracts",
  ]) {
    assert.doesNotMatch(
      dockerignore,
      new RegExp(`^!${forbidden.replaceAll("/", "\\/")}`, "m")
    );
  }
  for (const generated of [
    ...
    assert.match(
      dockerignore,
      new RegExp(`^${generated.replaceAll("/", "\\/")}$`, "m")
    );
  }
});
