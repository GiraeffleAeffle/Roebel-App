import { departmentLabel, departmentSummaryText } from "@roebel/stadtstack-federation-client";
...
export function BriefResponses({ responses, collapsible = false }: { responses: BriefResponse[]; collapsible?: boolean }) {
  return <div className="grid gap-3 sm:grid-cols-2">{responses.map(item => {
    const contents = <><p className="mt-3 whitespace-pre-wrap text-sm leading-6">{departmentSummaryText(item.publicSummary)}</p>
    ...
    return collapsible ? <details key={item.departmentId} className="self-start rounded-xl border bg-card p-4">
      <summary className="cursor-pointer font-semibold">{departmentLabel(item.departmentId)} <span className="ml-1 text-xs font-normal text-emerald-700">Geprüft</span></summary>{contents}
