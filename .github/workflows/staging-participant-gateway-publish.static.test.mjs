...
test("the gateway publisher carries the citizen-adoption migration and contract into v3 release evidence", () => {
  for (const path of [
  ...
  assert.match(workflow, /roebel_staging_participant_gateway_release_pins_v3/u);
  assert.match(workflow, /roebel_staging_publication_receipt_v3/u);
  assert.match(
    workflow,
